// Package idealista parses idealista.com alert emails into listings.
package idealista
...
// titleLinkRe matches a listing's title anchor: of the several anchors
// pointing at the same listing (photo, "Ver N fotos" CTA, title, "Contactar"),
// only the title anchor carries a title="..." attribute directly on the <a>
// (the photo anchor's title attribute is on its nested <img>, and the CTA/
// contact anchors have none at all).
var (
	titleLinkRe = regexp.MustCompile(`<a href="([^"]*inmueble/(\d+)/[^"]*)" title="([^"]+)">`)
	priceRe     = regexp.MustCompile(`>([\d.,]+)\s*€`)
	roomsSizeRe = regexp.MustCompile(`(\d+)\s*hab\.\s*(\d+)\s*m`)
)
...
// parsePrice converts a Spanish-formatted price ("1.300" or "1.300,50") into
